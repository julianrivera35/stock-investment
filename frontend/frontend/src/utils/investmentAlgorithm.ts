...
function getPremiumBrokerages(recommendations: Recommendation[]): string[] {
  const brokerageStats: Record<string, {
    name: string
    totalRecommendations: number
  ...
  Object.keys(brokerageStats).forEach(brokerageName => {
    const brokerageRecs = recommendations.filter(r => r.brokerage?.name === brokerageName)
    const upgrades = brokerageRecs.filter(r => 
      r.action.toLowerCase().includes('raised') || 
      r.action.toLowerCase().includes('upgrade')
    ).length

    brokerageStats[brokerageName].accuracyScore = brokerageRecs.length > 0 
      ? (upgrades / brokerageRecs.length) * 100 
      : 0
    
    brokerageStats[brokerageName].uniqueCompanies = brokerageCompanies[brokerageName]?.size || 0
  })
...
export function analyzeStockInvestments(recommendations: Recommendation[]): StockScore[] {
...
function groupRecommendationsByStock(recommendations: Recommendation[]): Record<string, Recommendation[]> {
...
function analyzeStock(ticker: string, recommendations: Recommendation[], premiumBrokerages: string[]): StockScore {
  ...
  const averageTargetPrice = validTargets.length > 0 
    ? validTargets.reduce((sum, price) => sum + price, 0) / validTargets.length
    : 0
  
  const currentTargetPrice = validTargets.length > 0 
    ? Math.max(...validTargets)
    : 0
  
  // Calculate target upside
  const minTargetPrice = validTargets.length > 0 ? Math.min(...validTargets) : 0
  const averageTargetUpside = minTargetPrice > 0 
    ? ((averageTargetPrice - minTargetPrice) / minTargetPrice) * 100
    : 0
  
  // Get unique brokerages
  const uniqueBrokerages = [...new Set(
    recommendations
  ...
  const daysFromLastRecommendation = latestRecommendation
    ? Math.floor((Date.now() - latestRecommendation.getTime()) / (1000 * 60 * 60 * 24))
    : 999
  
  // Calculate metrics (0-100 scale)
  const metrics = {
    recommendationCount: Math.min((recommendations.length / 20) * 100, 100),
    upgradeRatio: recommendations.length > 0 ? (upgrades.length / recommendations.length) * 100 : 0,
    averageTargetUpside: Math.min(Math.max(averageTargetUpside, 0), 100),
    analystConsensus: calculateConsensusScore(upgrades.length, downgrades.length, reiterations.length),
    recencyScore: Math.max(100 - (daysFromLastRecommendation * 2), 0),
    brokerageQuality: calculateBrokerageQualityScore(recommendations, premiumBrokerages)
  }
  ...
  const weights = {
    ...
    upgradeRatio: 0.25,
    averageTargetUpside: 0.20,
    analystConsensus: 0.20,
    recencyScore: 0.10,
    brokerageQuality: 0.10
  }
...
function calculateConsensusScore(upgrades: number, downgrades: number, reiterations: number): number {
...
function calculateBrokerageQualityScore(recommendations: Recommendation[], premiumBrokerages: string[]): number {
...
export function getTopInvestmentPicks(recommendations: Recommendation[], count: number = 5): StockScore[] {
