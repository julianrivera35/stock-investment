  ...
  <div class="space-y-6">
    ...
    <div class="card p-6">
      <div class="flex items-center justify-between mb-4">
        <div>
          ...
          <p class="text-sm text-gray-600 mt-1">
            ...
            {{ store.recommendations.length.toLocaleString() }} analyst
            recommendations
          </p>
        </div>
      ...
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <div
          class="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-blue-800"
        >
          <div>
            • <strong>Recommendation Volume</strong> (15%): More analyst
            coverage = higher confidence
          </div>
          <div>
            • <strong>Upgrade Ratio</strong> (25%): Percentage of positive
            rating changes
          </div>
          <div>
            • <strong>Target Upside</strong> (20%): Potential price appreciation
          </div>
          <div>
            • <strong>Analyst Consensus</strong> (20%): Agreement among analysts
          </div>
          <div>
            • <strong>Recency</strong> (10%): How recent the recommendations are
          </div>
          <div>
            • <strong>Brokerage Quality</strong> (10%): Reputation of
            recommending firms
          </div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
          ...
          <div class="text-center p-3 bg-gray-50 rounded-lg">
            <div class="text-lg font-bold text-purple-600">
              {{ stock.details.uniqueBrokerages }}
            </div>
        ...
        <div class="space-y-2">
          ...
          <div class="space-y-1">
            <div class="flex justify-between items-center">
              ...
              <span class="text-xs font-medium"
                >{{ stock.metrics.upgradeRatio.toFixed(1) }}%</span
              >
            </div>
          ...
          <div class="space-y-1">
            <div class="flex justify-between items-center">
              ...
              <span class="text-xs font-medium"
                >{{ stock.metrics.analystConsensus.toFixed(1) }}%</span
              >
            </div>
            <div class="w-full bg-gray-200 rounded-full h-1.5">
              <div
          ...
          <div class="space-y-1">
            <div class="flex justify-between items-center">
              ...
              <span class="text-xs font-medium"
                >{{ stock.metrics.brokerageQuality.toFixed(1) }}%</span
              >
            </div>
            <div class="w-full bg-gray-200 rounded-full h-1.5">
              <div
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr
              v-for="(stock, index) in allAnalysis"
              :key="stock.ticker"
              class="hover:bg-gray-50"
            >
              <td class="table-cell">
                <span :class="getRankTextClass(index)" class="font-medium">
                  #{{ index + 1 }}
                </span>
              </td>
              <td class="table-cell">
...
import { analyzeStockInvestments } from "@/utils/investmentAlgorithm";
