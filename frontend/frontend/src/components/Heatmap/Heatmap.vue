<template>
  <div class="card">
    <div class="p-6 border-b border-gray-200">
      <h3 class="text-lg font-semibold text-gray-900 mb-2">Recommendations Heatmap</h3>
      <p class="text-sm text-gray-600">Number of recommendations by brokerage and company</p>
    </div>
    ...
    <div class="p-6">
      ...
      <div class="mb-6 flex flex-wrap gap-4">
        <div>
          ...
          <select v-model="topCompaniesCount" class="px-3 py-2 border border-gray-300 rounded-lg text-sm">
        ...
        <div>
          ...
          <select v-model="topBrokeragesCount" class="px-3 py-2 border border-gray-300 rounded-lg text-sm">
            <option :value="5">Top 5</option>
            <option :value="10">Top 10</option>
            <option :value="15">Top 15</option>
          </select>
        ...
        <div>
          ...
          <select v-model="colorScale" class="px-3 py-2 border border-gray-300 rounded-lg text-sm">
      ...
      <div class="overflow-x-auto">
        <div class="inline-block min-w-full">
          ...
          <div class="flex">
            <!-- Empty corner cell -->
            <div class="w-32 h-12 border border-gray-200 bg-gray-50 flex items-center justify-center">
          ...
          >
            <!-- Brokerage name cell -->
            <div class="w-32 h-12 border border-gray-200 bg-gray-50 flex items-center px-2">
              <span 
      ...
      <div class="mt-6 flex items-center justify-between">
        <div class="flex items-center space-x-4">
          ...
          <div class="flex items-center space-x-2">
            <div class="w-4 h-4 border border-gray-300" :style="{ backgroundColor: getColorForValue(0) }"></div>
            <span class="text-xs text-gray-500">0</span>
          </div>
          <div class="flex items-center space-x-2">
            <div class="w-4 h-4 border border-gray-300" :style="{ backgroundColor: getColorForValue(maxValue / 2) }"></div>
            <span class="text-xs text-gray-500">{{ Math.round(maxValue / 2) }}</span>
          </div>
          <div class="flex items-center space-x-2">
            <div class="w-4 h-4 border border-gray-300" :style="{ backgroundColor: getColorForValue(maxValue) }"></div>
      ...
      <div v-if="selectedCell" class="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <div class="mt-3">
          ...
          <div class="flex gap-2 mt-1">
            <span class="badge-upgrade">{{ selectedCell.upgrades }} upgrades</span>
            <span class="badge-downgrade">{{ selectedCell.downgrades }} downgrades</span>
            <span class="badge-reiterate">{{ selectedCell.reiterations }} reiterations</span>
          </div>
        </div>
      </div>
    </div>
...
const topCompanies = computed(() => {
  const companyCounts: Record<string, { ticker: string, name: string, count: number }> = {}
...
function getColorForValue(value: number): string {
  ...
  const r = Math.round(colorConfig.light[0] + (colorConfig.dark[0] - colorConfig.light[0]) * intensity)
  const g = Math.round(colorConfig.light[1] + (colorConfig.dark[1] - colorConfig.light[1]) * intensity)
  const b = Math.round(colorConfig.light[2] + (colorConfig.dark[2] - colorConfig.light[2]) * intensity)
...
function showCellDetails(brokerage: string, ticker: string) {
  ...
  const downgrades = recommendations.filter(r => 
    r.action.toLowerCase().includes('lowered') || r.action.toLowerCase().includes('downgrade')
  ).length
