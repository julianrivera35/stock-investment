  ...
  <div class="card overflow-hidden">
    ...
    <div class="p-6 border-b border-gray-200">
      <div class="flex flex-col sm:flex-row gap-4">
        ...
        <div class="flex gap-2">
          <select v-model="selectedBrokerage" class="px-3 py-2 border border-gray-300 rounded-lg text-sm">
          ...
          <select v-model="selectedAction" class="px-3 py-2 border border-gray-300 rounded-lg text-sm">
    ...
    <div class="overflow-x-auto">
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            <th @click="sortBy('company')" class="table-header cursor-pointer hover:bg-gray-100">
              <div class="flex items-center">
                Company
                <span class="ml-1">{{ getSortIcon('company') }}</span>
              </div>
            </th>
            <th @click="sortBy('brokerage')" class="table-header cursor-pointer hover:bg-gray-100">
              <div class="flex items-center">
                Brokerage
                <span class="ml-1">{{ getSortIcon('brokerage') }}</span>
              </div>
            </th>
            <th @click="sortBy('action')" class="table-header cursor-pointer hover:bg-gray-100">
              <div class="flex items-center">
                Action
            ...
            <th @click="sortBy('time')" class="table-header cursor-pointer hover:bg-gray-100">
              <div class="flex items-center">
                Date
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="recommendation in paginatedRecommendations" :key="recommendation.id" class="hover:bg-gray-50">
            <!-- Company -->
            <td class="table-cell">
              <div class="flex items-center">
                ...
                <div>
                  <div class="font-medium text-gray-900">{{ recommendation.company.name }}</div>
                  <div class="text-sm text-gray-500 font-mono">{{ recommendation.company.ticker }}</div>
                </div>
              </div>
            </td>
            ...
            <!-- Rating Change -->
            <td class="table-cell">
              <div v-if="recommendation.rating_from && recommendation.rating_to">
                <span class="text-gray-600">{{ recommendation.rating_from }}</span>
                <span class="mx-2 text-gray-400">→</span>
                <span class="font-medium text-gray-900">{{ recommendation.rating_to }}</span>
              </div>
            ...
            <!-- Price Target Change -->
            <td class="table-cell">
              <div v-if="recommendation.target_from && recommendation.target_to">
                <div class="flex items-center">
                  <span class="text-gray-600">${{ recommendation.target_from.toFixed(2) }}</span>
                  <span class="mx-2 text-gray-400">→</span>
                  <span class="font-medium text-gray-900">${{ recommendation.target_to.toFixed(2) }}</span>
                  <span 
                    :class="getPriceChangeClass(recommendation.target_from, recommendation.target_to)"
                    class="ml-2 text-xs"
                  >
                    {{ getPriceChangeText(recommendation.target_from, recommendation.target_to) }}
                  </span>
                </div>
              </div>
              <span v-else class="text-gray-400">N/A</span>
            ...
            <!-- Date -->
            <td class="table-cell">
              <div class="text-gray-900">{{ formatDate(recommendation.time) }}</div>
              <div class="text-xs text-gray-500">{{ formatTime(recommendation.time) }}</div>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
      <div class="text-sm text-gray-700">
        Showing {{ ((currentPage - 1) * pageSize) + 1 }} to {{ Math.min(currentPage * pageSize, filteredRecommendations.length) }} 
...
const filteredRecommendations = computed(() => {
  ...
  if (selectedBrokerage.value) {
    filtered = filtered.filter(r => r.brokerage?.name === selectedBrokerage.value)
  }
...
const totalPages = computed(() => Math.ceil(filteredRecommendations.value.length / pageSize.value))
...
function getActionBadgeClass(action: string) {
  ...
  } else if (actionLower.includes('lowered') || actionLower.includes('downgrade')) {
